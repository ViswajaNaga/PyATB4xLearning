...
class Animal(ABC):
    ...
    @abstractmethod # this is like a annotation( this is incomplete method)
    def sound(self):
        pass#TypeError: Can't instantiate abstract class Dog without an implementation for abstract method 'sound'
# as the method sound is incomplete in nature in Animal class, somebody has to complete it(this menthod)(override) in different class
class Dog(Animal):
    # pass
    def sound(self):
